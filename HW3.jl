...
# ╔═╡ b26b0b4e-2896-4b0b-a4dd-3ea16ee16efa
...
# ╔═╡ 2b369acb-b20c-4a82-baec-034a0181ab55
table = transition_frequencies(first_sample)

# ╔═╡ 79ddf4ef-c023-4c66-a80b-39dccb376de1
function letter_of_index(index)
	x = [(i, j) for i in 1:27, j in 'a':'z']
	return x[index][2]
end

# ╔═╡ ed2d4241-291a-44d3-b35d-7cbdefd45a36
# double_letters = filter(!iszero, [table[i, i] for i in 1:size(table)[1]])
double_letters =[i for i in filter(!isnothing, [if table[index_of_letter(i), index_of_letter(i)] != 0 i end for i in alphabet])]
...
begin
	...
	l = 'a'
	for i in 'a':'z'
		if table[index_of_letter('w'),index_of_letter(i)] > y
			l = i
		end
	end
	most_likely_to_follow_w = l
end
...
begin
	...
	letter = 'a'
	for i in alphabet
		if table[index_of_letter(i), index_of_letter('w')] > x
			letter = i
		end
	end
