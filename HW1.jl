...
# ╔═╡ 01aa8a25-57d2-495a-acba-f0fe92b257be
md"""
Create the colored_line function
"""
...
# ╔═╡ 50e4761b-4b0e-445e-a578-a25aa76df51d
function get_greens(image::AbstractMatrix)
	return get_green.(image)
end

# ╔═╡ 81bb635e-640e-4435-b700-908b3c658a17
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ e0f898da-892b-4291-b989-ba6166b4189d
function get_blues(image::AbstractMatrix)
	return get_blue.(image)
end

# ╔═╡ c8f1e5a9-f1e4-494d-ab3a-eb4739357a3f
RGB.( 0, get_greens(philip_head), 0)

# ╔═╡ fbeea1fb-0f33-43c1-9942-ab1d2f241c6f
RGB.( 0, 0, get_blues(philip_head))

# ╔═╡ dd38797f-28fd-4961-9a9f-073ae921b119
...
function mean_color(image)
	mean = RGB( sum(get_reds(image))/length(image), sum(get_greens(image)/length(image)), sum(get_blues(image))/length(image) )
	return mean
end
...
function noisify(x::Number, s)
	return clamp( rand(-s:0.00001:s) + x, 0, 1)
end
...
The image is unrecognisable with intensity greater than 1.5, because the greater than 1 the noise parameter the more probable it is to catch a number outside of the [0,1] range and when the noisify function finds a number outside this range then it just converts it to 0 or 1.
"""
...
function custom_filter(pixel::AbstractRGB)#√pixel
