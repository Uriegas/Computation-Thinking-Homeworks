...
filter(j -> j*(1/j) ≠ 1 , [j for j ∈ 1:1000 ])
...
ten_twelve = FirstRankOneMatrix([j for j ∈ 1:10], [j for j ∈ 1:12]) # Your code here
...
begin
	...
	RankOneMatrix(v, w) = RankOneMatrix(v, w)
end
...
function Base.getindex(M::LowRankMatrix, i, j)
	return sum([getindex(M.Ms[k], i, j) for k ∈ 1:M.rank])
end

# ╔═╡ c784e02c-8502-11eb-3efa-7f4c45f4274c
function Base.getindex(M::RankTwoMatrix, i, j)
	return Base.getindex(M.A, i, j) + Base.getindex(M.B, i, j)
end
...
Because the RankOneMatrix saves $2nk$ while a dense matrix $n^2$, solving for k:
...
# ╔═╡ 2dd18833-ee83-49c8-927f-829571513461
svd(rand(4,3))
...
function LinearAlgebra.svd(A::RankOneMatrix)
	...
	return LinearAlgebra.svd(collect(A))
end
...
function numerical_rank(A::AbstractMatrix; tol=1e-5)
	return length(filter(x ->  ( x > tol ), LinearAlgebra.svd(A).S))
end
