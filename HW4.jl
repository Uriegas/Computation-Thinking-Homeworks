...
# ╔═╡ 729532ee-c1d6-4b4b-98cc-357eb4470606
function get_neighbors_below(pixel::Vector, matrix::Matrix)
	m,n = size(matrix)
	# return minimum( [ [pixel[1]+1, clamp(pixel[2]+i, 1, n)] for i in -1:1 ] )
	return [clamp(pixel[2]+i, 1, n) for i in -1:1]

	# return reduce((a, b) ->[pixel[1]+1, clamp(pixel[2]+i, 1, n)], for i in -1:1)
end
...
# ╔═╡ 907fb406-a2af-4500-9e76-a343c8788feb
similar(1:5)
...
# ╔═╡ 607a2698-3f57-48b5-bb2d-6abae48d12c4
get_neighbors_below([2,1], grant_example)
...
# ╔═╡ 377a9d0b-a5a2-46b1-993a-593fd3170e95
begin
	i = [1,1]
	typeof(i)
	grant_example[:,1]
